...
import GuestAnalytics from "../Guest/Analytics.jsx";
...
function AnalyticsGuestPart() {
  const { customAlert } = React.useContext(AlertContext);
  const state = useLocation();
  const users = state.state.allGuest; // accessed by which page (login/register)
  ...
  const handleDetailClick =(host_email) => {
  ...
  const handleSearchChange =(event) => {
    ...
    // If user don't search, it will select first user of the list
  ...
  const handleDelteUser =(host_email)=>{
    ...
    try{
      ...
      state.state.allGuest = updatedUsers;
      ...
      <div style={{ display: "flex" }}>
        ...
        >
          ...
          <List>
            {users && users.map((user, index) => {
              ...
              return (
                <ListItem
                  ...
                  sx={{
                    backgroundColor:
                      selectedHost === email ? "#e5e5e5" : "white",
                  }}
                >
                  <ListItemButton
                    onClick={() => setSelectedHost(email)}
                    disableRipple
                  >
            ...
            {!users ? "There are no guests." : null}
          </List>
        </Box>
        ...
        <div style={{ padding: "0px 20px" }}>
          {selectedHost ? (
            <>
              <h2>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{selectedHost} &nbsp;&nbsp;
                <MyButton onClick={() => handleDetailClick(selectedHost)}>Details</MyButton>   &nbsp;&nbsp;
                <MyButton onClick={()=> handleDelteUser(selectedHost)}>Delete This User</MyButton>
              </h2>
              <GuestAnalytics userEmail={selectedHost} />
            </>
          ) : null }
...
export default AnalyticsGuestPart;
